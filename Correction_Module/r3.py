...
def rigids_from_3_points(
    point_on_neg_x_axis: Vecs,  # shape (...)-x轴上的点
    origin: Vecs,  # shape (...)原点
    point_on_xy_plane: Vecs,  # shape (...)xy平面上的点
) -> Rigids:  # shape (...)
...
def rots_mul_vecs(m: Rots, v: Vecs) -> Vecs:
  ...
  #print("fffff",torch.tensor(m.xx),'\n',torch.tensor(m.xy) * v.y,'\n',torch.tensor(m.xx) * v.x + torch.tensor(m.xy) * v.y + torch.tensor(m.xz) * v.z)
  return Vecs(torch.tensor(m.xx,dtype=torch.float32,requires_grad=True) * v.x + torch.tensor(m.xy,dtype=torch.float32,requires_grad=True) * v.y + torch.tensor(m.xz,dtype=torch.float32,requires_grad=True) * v.z,
              torch.tensor(m.yx,dtype=torch.float32,requires_grad=True) * v.x + torch.tensor(m.yy,dtype=torch.float32,requires_grad=True) * v.y + torch.tensor(m.yz,dtype=torch.float32,requires_grad=True) * v.z,
              torch.tensor(m.zx,dtype=torch.float32,requires_grad=True) * v.x + torch.tensor(m.zy,dtype=torch.float32,requires_grad=True) * v.y + torch.tensor(m.zz,dtype=torch.float32,requires_grad=True) * v.z)
